  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const destination = result.kind === "donation"
          ? "/donate?status=" + (result.ok ? "thanks" : "cancelled") + "&ref=" + encodeURIComponent(reference)
          : "/dashboard?payment=" + (result.ok ? "success" : "cancelled");
